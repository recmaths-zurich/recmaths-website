...
class ComplexNumber {
    ...
    // Γ(z) — principal branch, fast real path + complex support
    gamma() {
        ...
        // Complex path: reflection for Re(z) < 0.5
    ...
    // Real-number Lanczos Γ (used by gamma() fast path)
    static _gammaReal(x) {
        const p = ComplexNumber._lanczos(), g = 7;
    ...
    // z! = Γ(z + 1). Integer fast path; throws on negative integers.
    factorial() {
    ...
    round() {
        return new ComplexNumber(Math.round(this.realPart), Math.round(this.imaginaryPart))
    }

    floor() {
        return new ComplexNumber(Math.floor(this.realPart), Math.floor(this.imaginaryPart))
    }

    ceil() {
        return new ComplexNumber(Math.ceil(this.realPart), Math.ceil(this.imaginaryPart))
    }
    ...
    imag() {
        return new ComplexNumber(this.imaginaryPart, 0)
    }
...
function evaluateNumberString(numberString) {
    ...
    const allowedFunctions = {
        ...
        "floor":     {compute: n => ComplexNumber.asComplexNumber(n).floor()},
        "ceil":      {compute: n => ComplexNumber.asComplexNumber(n).ceil()},
        "round":     {compute: n => ComplexNumber.asComplexNumber(n).round()},
        "real":      {compute: n => ComplexNumber.asComplexNumber(n).real()},
        "imag":      {compute: n => ComplexNumber.asComplexNumber(n).imag()},
